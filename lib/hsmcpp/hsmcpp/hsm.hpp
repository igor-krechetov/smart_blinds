                                                                         ...
                                                                         HsmStateExitCallback_t onExiting)
{
    ...
    if (onStateChanged || onEntering || onExiting)
    {
        ...
        __HSM_TRACE_CALL_ARGS__("mRegisteredStates.size=%ld", mRegisteredStates.size());
    }
}
    ...
    if (parent != substate)
    {
        ...
        if (false == hasParentState(substate, prevState))
        {
            ...
            while (true == hasParentState(curState, prevState))
            {
                ...
                {
                    __HSM_TRACE_CALL_ARGS__("requested operation will result in substates recursion (parent=<%s>, substate=<%s>)",
        ...
        {
            __HSM_TRACE_CALL_ARGS__("substate <%s> already has a parent <%s>",
...
bool HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::registerStateAction(const HsmStateEnum state,
                                                                               ...
                                                                               Args... args)
{
    __HSM_TRACE_CALL_ARGS__("state=<%s>, actionTrigger=%d, action=%d",
...
bool HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::transitionEx(const HsmEventEnum event,
                                                                        ...
                                                                        Args... args)
{
    __HSM_TRACE_CALL_ARGS__("transitionEx: event=<%s>, clearQueue=%s, sync=%s",
...
{
    __HSM_TRACE_CALL_ARGS__("event=<%s>", getEventName(event).c_str());
...
void HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::transitionWithArgsArray(const HsmEventEnum event,
                                                                                   const VariantVector_t& args)
{
    __HSM_TRACE_CALL_ARGS__("event=<%s>, args.size=%lu", getEventName(event).c_str(), args.size());
...
bool HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::transitionExWithArgsArray(const HsmEventEnum event,
                                                                                     ...
                                                                                     const VariantVector_t& args)
{
    __HSM_TRACE_CALL_ARGS__("event=<%s>, clearQueue=%s, sync=%s, args.size=%lu",
...
{
    __HSM_TRACE_CALL_ARGS__("mActiveStates.size=%ld", mActiveStates.size());
    ...
    {
        {
            ...
            for (auto it = mActiveStates.begin(); it != mActiveStates.end(); ++it)
            {
                __HSM_TRACE__("state=<%s>", getStateName(*it).c_str());
...
void HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::dispatchEvents()
{
    ...
    if (false == mStopDispatching)
    {
        if (false == mPendingEvents.empty())
        {
            ...
            __HSM_TRACE__("unlock with status %d", SC2INT(transitiontStatus));
...
{
    __HSM_TRACE_CALL_ARGS__("id=%d", SC2INT(id));
...
{
    __HSM_TRACE_CALL_ARGS__("state=<%s>", getStateName(state).c_str());
    bool res = true;
...
void HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::executeStateAction(const HsmStateEnum state,
                                                                              const StateActionTrigger actionTrigger)
{
    __HSM_TRACE_CALL_ARGS__("state=<%s>, actionTrigger=%d", getStateName(state).c_str(), SC2INT(actionTrigger));
...
{
    __HSM_TRACE_CALL_ARGS__("parent=<%s>, child=<%s>", getStateName(parent).c_str(), getStateName(child).c_str());
...
{
    __HSM_TRACE_CALL_ARGS__("topLevelState=<%s>, activeStates.size=%ld",
    ...
    for (auto itActiveState = activeStates.begin(); itActiveState != activeStates.end(); ++itActiveState)
    {
        ...
        {
            __HSM_TRACE__("curState=<%s>, parentState=<%s>",
                                getStateName(curState).c_str(), getStateName(parentState).c_str());
            auto itRange = mHistoryStates.equal_range(parentState);

            if (itRange.first != itRange.second)
            {
                __HSM_TRACE__("parent=<%s> has history items", getStateName(parentState).c_str());
                ...
                for (auto it = itRange.first; it != itRange.second; ++it)
                {
                    ...
                    if (itCurHistory != mHistoryData.end())
                    {
                        ...
                        {
                            __HSM_TRACE__("SHALLOW -> store state <%s> in history of parent <%s>",
                        ...
                        {
                            __HSM_TRACE__("DEEP -> store state <%s> in history of parent <%s>",
...
bool HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::findTransitionTarget(const HsmStateEnum fromState,
                                                                                ...
                                                                                std::list<TransitionInfo>& outTransitions)
{
    __HSM_TRACE_CALL_ARGS__("fromState=<%s>, event=<%s>", getStateName(fromState).c_str(), getEventName(event).c_str());
    bool continueSearch;
    ...
    do
    {
        ...
        else
        {
            for (auto it = itRange.first; it != itRange.second; ++it)
            {
                __HSM_TRACE__("check transition to <%s>...", getStateName(it->second.destinationState).c_str());
                    ...
                    (it->second.expectedConditionValue == it->second.checkCondition(transitionArgs)))
                {
                    ...
                    do
                    {
                        ...
                        if (true == hasSubstates(currentParent))
                        {
                            ...
                            {
                                __HSM_TRACE__("state <%s> has entrypoints", getStateName(currentParent).c_str());
                                std::list<HsmStateEnum> entryPoints;
...
{
    __HSM_TRACE_CALL_ARGS__("event=<%s>, transitionType=%d", getEventName(event.type).c_str(), SC2INT(event.transitionType));
...
{
    __HSM_TRACE_CALL_ARGS__("activeState=<%s>, event=<%s>, transitionType=%d",
    ...
    {
        __HSM_TRACE__("forced history transitions: %d", SC2INT(event.forcedTransitionsInfo->size()));
                ...
                if (it->fromState != it->destinationState)
                {
                    ...
                    if (true == onStateEntering(it->destinationState, event.args))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                __HSM_TRACE__("state=<%s> is a history state with %ld stored states",
                                ...
                                else
                                {
                                    ...
                                    if (INVALID_HSM_STATE_ID == itHistoryData->second.defaultTarget)
                                    {
                                        ...
                                        {
                                            __HSM_TRACE__("found parent=<%s> for history state=<%s>",
                            ...
                            {
                                __HSM_TRACE__("state <%s> has substates with %d entry points (first: <%s>)",
                                                    getStateName(it->destinationState).c_str(),
    ...
    {
        __HSM_TRACE__("event <%s> in state <%s> was ignored.",
...
{
    __HSM_TRACE_CALL_ARGS__("clearPendingEvents: mPendingEvents.size()=%ld", mPendingEvents.size());
...
HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::PendingEventInfo::~PendingEventInfo()
{
    ...
    {
        __HSM_TRACE_CALL_ARGS__("event=<%d> was deleted. releasing lock", SC2INT(type));
...
void HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::PendingEventInfo::releaseLock()
{
    if (isSync())
    {
        __HSM_TRACE_CALL_ARGS__("releaseLock");
        unlock(HsmEventStatus_t::DONE_FAILED);
        cvLock.reset();
        syncProcessed.reset();
    }
...
void HierarchicalStateMachine<HsmStateEnum, HsmEventEnum>::PendingEventInfo::wait(const int timeoutMs)
{
    if (isSync())
    {
        ...
        __HSM_TRACE_CALL_ARGS__("trying to wait... (current status=%d, %p)", SC2INT(*transitionStatus), transitionStatus.get());
        if (timeoutMs > 0)
        {
        ...
        __HSM_TRACE__("unlocked! transitionStatus=%d", SC2INT(*transitionStatus));
    }
}
...
{
    __HSM_TRACE_CALL_ARGS__("try to unlock with status=%d", SC2INT(status));
...
{
    __HSM_TRACE_CALL_ARGS__("oldState=<%s>, newState=<%s>",
...
{
    __HSM_TRACE_CALL_ARGS__("newState=<%s>", getStateName(newState).c_str());
    bool wasAdded = false;
    ...
    __HSM_TRACE__("mActiveStates.size=%d", SC2INT(mActiveStates.size()));
    return wasAdded;
}
